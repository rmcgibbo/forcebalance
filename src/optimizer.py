...
class Optimizer(ForceBalanceBaseClass):
        ...
        def xwrap(func,verbose=True):
            def my_func(mvals):
                ...
                if Answer < my_func.x_best or my_func.x_best == None:
                    ...
                    color = "\x1b[91m"
                if verbose:
                    logger.debug( "k=" + ' '.join(["% .4f" % i for i in mvals]))
                    logger.debug("X2= %s%12.3e\x1b[0m d(X2)= %12.3e\n" % (color,Answer,dx))
                return Answer
            my_func.x_best = None
            return my_func
    ...
    def Scan_Values(self,MathPhys=1):
        ...
        try:
            ...
            logger.critical("Syntax error: in the input file please use scan_vals low:hi:nsteps\n")
            sys.exit(1)
        if len(vals_in) != 3:
            logger.critical("Syntax error: in the input file please use scan_vals low:hi:nsteps\n")
            sys.exit(1)
    ...
    def SinglePoint(self):
        ...
        logger.info("The objective function is:" + data['X'] + '\n')
    ...
    def readchk(self):
        ...
        if self.rchk_fnm != None:
            ...
            if os.path.exists(absfnm):
                ...
                logger.warning("\x1b[40m\x1b[1;92mWARNING:\x1b[0m read_chk is set to True, but checkpoint file not loaded (wrong filename or doesn't exist?)\n")
        return self.chk
